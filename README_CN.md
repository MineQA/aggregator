...
当前文档已按上游最新版本更新，重点变更如下：

- **配置结构调整**：`process.py` 使用 `storage` + `groups` 管理推送后端与输出分组；旧版 `push` 配置需参考 `subscribe/config/config.default.json` 迁移。
- **输出格式统一**：`collect.py` 使用 `-t/--targets` 指定输出格式，默认生成 `clash`、`v2ray`、`singbox`；旧的 `--both`、`--mixed` 已由该参数统一替代。
- **新增地理规范化**：新增 `subscribe/location.py`，支持基于 GeoLite2 数据库识别节点地区、家庭宽带类型并规范化节点命名。
- **新增脚本与工具**：新增 FOFA 插件、动态爬取插件、配置清理工具、3X-UI 面板扫描工具、SOCKS 检测工具等，详见 [工具脚本](#工具脚本)。
- **二进制命名调整**：Clash/Mihomo 与 subconverter 的平台二进制文件改为带架构后缀，如 `clash-linux-amd`、`subconverter-linux-amd`。
...
ENABLE_SPECIAL_PROTOCOLS=true           # 启用 VLESS/TUIC/Hysteria/Hysteria2 等特殊协议
TRACE_ENABLE=false                      # 启用 HTTP 请求追踪日志
...
> 注意：`process.py` 的环境文件参数为 `-e/--environment`。历史版本中曾出现过 `--envrionment` 拼写，建议统一使用当前参数名。
...
> 注意：`collect.py` 的输出格式由 `-t/--targets` 统一控制，默认值为 `clash v2ray singbox`。旧版 `--both`、`--mixed` 不再作为主要用法。
...
| `api_prefix` | string | 可选 | `""` | 面板 API 前缀；用于兼容 `/api/v1/`、`/api?scheme=` 等非标准入口 |
...
该能力由 `subscribe/location.py` 提供，会基于 GeoLite2 数据库对节点 IP 做地区识别，并可在节点重命名时补充国家/地区、住宅宽带等信息。首次使用时可能需要下载地理数据库，请确保运行环境可访问相关下载地址。
...
除主入口 `process.py` 与 `collect.py` 外，上游新增或强化了若干辅助脚本，适合在清洗配置、扫描面板或扩展爬取源时使用。

### FOFA 插件 (`subscribe/scripts/fofa.py`)

FOFA 插件通过 `fofa-hack` 搜索公网 Clash 配置页面，并尝试提取订阅链接或二次解析页面内容。
...
> FOFA 插件依赖 `requirements.txt` 中的 `fofa-hack`。如搜索结果为空，请优先检查网络、查询额度和依赖安装情况。

### 动态爬取插件 (`subscribe/scripts/dynamic.py`)

动态插件用于处理需要运行时动态发现、提取或二次解析的订阅来源。建议作为脚本插件放入 `crawl.scripts`，并通过 `params.config.push_to` 指定输出分组。

### 配置清理工具 (`tools/clean.py`)

用于清洗 Clash YAML 配置，支持按 `server:port` 去重、可选地理位置重命名、强制 TLS/安全设置、同名节点编号填充和备份原文件。

```bash
python tools/clean.py -c proxies.yaml -w ./output -b
```
...
| 参数 | 说明 |
| ---- | ---- |
| `-c, --config` | 待清理的 Clash 配置文件 |
| `-l, --location` | 启用地理位置重命名 |
| `-s, --secure` | 强制使用更安全的 TLS/证书设置 |
| `-n, --num` | 编号补齐位数 |
| `-u, --update` | 更新地理数据库或相关资源 |
| `-w, --workspace` | 工作目录 |
| `-b, --backup` | 清理前备份原文件 |

### 3X-UI 面板扫描工具 (`tools/xui.py`)

用于批量扫描 3X-UI 面板，尝试 Cookie 登录并提取 `vless`、`vmess`、`trojan`、`ss` 等入站配置，可输出 Base64 订阅链接列表和 Markdown 表格。

```bash
python tools/xui.py -f panels.txt -a availables.txt -l links.txt -m table.md
```
...
| 参数 | 说明 |
| ---- | ---- |
| `-f, --filename` | 面板地址列表文件，必需 |
| `-a, --available` | 保存可登录面板账号密码的文件名，默认 `availables.txt` |
| `-l, --link` | 保存提取出的订阅链接列表，默认 `links.txt` |
| `-m, --markdown` | 保存 Markdown 结果表格，默认 `table.md` |
| `-t, --thread` | 并发线程数 |
| `-u, --update` | 更新辅助资源 |
| `-w, --workspace` | 工作目录 |
| `-i, --invisible` | 隐藏进度条 |

### SOCKS 检测工具 (`tools/socks-checker.py`)

用于批量检测 SOCKS 代理可用性，适合在导入外部代理源前做连通性过滤。具体参数请以 `python tools/socks-checker.py -h` 输出为准。

### 地理位置模块 (`subscribe/location.py`)

该模块主要供 `groups[].regularize`、`tools/clean.py` 等功能调用，也可作为节点命名规范化能力的基础模块。它负责：

- 下载和读取 GeoLite2-City 数据库；
- 将 IP 映射为国家/地区并转换为中文命名；
- 检测住宅宽带等节点属性；
- 对同名节点按配置位数补齐编号。

### 运行注意事项

- 新增依赖已写入 `requirements.txt`，更新代码后请重新执行 `pip install -r requirements.txt`。
- 若使用 Docker，可通过构建参数 `PIP_INDEX_URL` 指定 pip 镜像源。
- 二进制文件名已按平台和架构拆分，旧部署脚本如写死 `clash-linux`、`subconverter-linux`，需要改为新的 `*-amd` 或 `*-arm` 名称。
- 使用 `process.py` 时，旧版 `push` 配置请迁移为 `storage` + `groups`；否则会出现存储项缺失或无法推送。
